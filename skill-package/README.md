...
> A self-contained Claude Code skill that gives Claude source-level knowledge of its own architecture — 172 lessons covering every internal subsystem, searchable three ways.
...
- **Layer 2** tokenizes your query and computes cosine similarity against pre-built TF-IDF vectors for all 172 lessons. Pure Node.js, no dependencies.
...
That's it. The zip contains everything — the SKILL.md brain, all 172 lessons, both search indexes, the lookup scripts, the README, and the LICENSE. No npm install, no server, no API keys.
...
```
  ...
  |                   |         |                   |         | Swarms & Workflows |
  | 172 lessons        |         | 10 cluster embeds |         | OpenClaw           |
...
| `claude-code-internals-lessons` | 172 lessons | Precise lesson-level search | ~6ms |
...
|   |   +-- semantic-index.json     TF-IDF vectors (172 lessons)
|   +-- scripts/
...
| `references/semantic-index.json` | 677KB | Pre-built TF-IDF vectors for all 172 lessons |
