...
> A self-contained Claude Code skill that gives Claude source-level knowledge of its own architecture — 197 lessons covering every internal subsystem, searchable three ways.
...
- **Layer 2** tokenizes your query and computes cosine similarity against pre-built TF-IDF vectors for all 197 lessons. Pure Node.js, no dependencies.
...
That's it. The zip contains everything the skill needs — the SKILL.md brain, all 197 lessons, both search indexes, the current-state layer, and the scripts. No npm install, no server, no API keys.
...
|   |   +-- semantic-index.json     TF-IDF vectors (197 lessons)
|   +-- scripts/
...
| `references/*.md` | 1.4MB | 50 chapter files holding all 197 lessons |
| `references/topic-index.json` | 341KB | Per-lesson bounds and keywords, plus a 4,480-key keyword map |
| `references/semantic-index.json` | 1.1MB | Pre-built TF-IDF vectors for all 197 lessons |
