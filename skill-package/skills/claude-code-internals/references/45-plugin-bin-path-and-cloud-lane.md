Updated: 2026-09-22 | Source: **live probes in three lanes** — a standalone CLI session (2.1.250, this machine), a Claude Cowork **local** session (host-loop VM shell, slug `nice-nifty-sagan`), and a Claude Cowork **cloud** session (`CLAUDE_CODE_ENTRYPOINT=remote_cowork`, `CLAUDE_CODE_VERSION=2.1.42`) — cross-read against CLI binaries 2.1.233/2.1.246/2.1.250 and `app.asar` 1.40609.0. Counts taken with `scripts/count-symbol.js` (ASCII + UTF-16LE, refuses a single total when a null-stripped extract disagrees). **Does NOT move the CLI content baseline.** First chapter here built primarily on *live session measurement* rather than artifact reading; static-only claims say so.
...
Re-checked 2026-09-22 against newer artifacts: still **0** in the Desktop-managed agents 2.1.260 and 2.1.275, the standalone CLI 2.1.278, and `app.asar` 2.2553.1. With the cloud agent dated at ≥ 2.1.248 (next section), "names an older agent reads" is ruled out as the explanation. Whatever consumes them is not any Anthropic client artifact on this machine — the environment runner is the remaining candidate, and it is not an artifact we hold.
...
`CLAUDE_CODE_VERSION` in a cloud shell or hook is **not the cloud agent's build**, and reading it as one contradicts `registry.as_of.container_cc_version_observed` (2.1.204–2.1.216, from session API traffic). Three independent checks:
...
**Consequence.** `state.container-agent-range` holds — the cloud lane runs a build the local artifacts do not pin — but the gap is *unpinned*, not *large*. A current-CLI claim still does not automatically describe the cloud lane.
...
- **Plugin MCP servers are not started by the container agent — the Desktop starts them on the host and bridges their tools in.** The spawn env carries `CLAUDE_CODE_SKIP_PLUGIN_MCP_SERVERS=1` and `…_EXCEPT=documents`; agent 2.1.278's `u2()` returns before reading a plugin's `.mcp.json`/inline `mcpServers` at all (transport-agnostic, `isBuiltin` exempt). That is one half of a two-half design. The other half is Desktop-side, present in every asar since **1.20186.0** and read here from 2.2553.1: `buildLocalMcpBridgeTools` (`[localMcpBridge]`) announces into a `cowork-remote` session the tools of two host-side pools — `claude_desktop_config.json` servers (`server_source:"user_config"`, `_meta["anthropic/kind"]:"local"`) and **plugin MCP servers** from `getPluginMcpInstance()` (`server_source:"plugin"`, kind `"plugin"`), each tool named `<server>__<tool>` with an `anthropic/approvalHash` that the Desktop's own tool-approval prompt gates. Plugin configs are collected by `[PluginBridgeMcp]` from the account's remote (`getHostPluginPaths`) and local (`getEnabledLocalPlugins`) Cowork plugins, behind **gate `3555657854`** (force-ON, `fr_mqzam1o7`, fcache 2026-09-22, 367 features) and the org policy `localMcpEnabled`/`allowedPluginMcpServers`; the exclusion list is served config **`227459766`** with shipped default **`["documents"]`** — "not bridging its MCP servers; the session-side copies serve instead" — the exact mirror of the agent's `_EXCEPT=documents`. Constraints (`[PluginMcpHostConfig]`, each a logged skip reason): the server must be a **valid stdio spawn config** (`invalid_config` otherwise — an HTTP `url` declaration is not bridged, which is why an HTTP server needs an `mcp-remote` stdio shim), must not reference `${user_config.*}` (`user_config_unsupported`), MCPB manifests must resolve to stdio with no un-defaulted required config; apply is bounded to 15 s then bridged with whatever registered (late results trigger `reloadRemoteToolsDevice("plugin_mcp_change")`), connect race 10 s, plugin request timeout 120 s, call timeout 180 s, telemetry `server_type:"bridge-stdio"`. Delivery is over the remote-tools device — the L126 `remote_devices` bridge — so **the Desktop must be open**. Live confirmation of the `user_config` pool: the `stackchan-mcp-mod` project drove a LAN robot from a cloud session on 2026-09-18 via `npx mcp-remote … --allow-http` in `claude_desktop_config.json` (`[LocalMcpServerManager] Connected to stackchan (N tools)` / `[localMcpBridge] announcing stackchan: N tool(s)`). The `plugin` pool is code-verified, not yet observed live.
