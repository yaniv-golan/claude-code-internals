...
- **An MCP server configured under the name `memory` is replaced** — for example `mcpServers.memory` in `claude_desktop_config.json`, a common name for the reference memory server — with only a warning in the Desktop log, and `memory` is added to `deniedMcpServers`. A *plugin*-declared server named `memory` is **not** affected: the Desktop's server map holds only config, connector and internal servers (plugins reach the agent through `--plugin-dir`), and the agent keys each plugin server as `plugin:<plugin>:<server>` (`aJn`) before checking `deniedMcpServers`, whose matcher compares the exact key (`s.serverName===e`). Such a plugin's tools appear as `mcp__plugin_<plugin>_memory__*` next to the relay's `mcp__memory__*`. The deny entry blocks only a non-SDK server loaded under the plain key `memory` — for example from `.mcp.json` or `--mcp-config`.
