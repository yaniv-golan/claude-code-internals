...
**The lane matters, and the table above is one lane.** Those 56 directories are all standalone-CLI sessions. The Cowork host-loop lane stages a plugin's files somewhere else entirely — under a `claude-hostloop-plugins/<hash>` root in the host temp tree (Ch17/L89, Ch35), where the hash is a **deterministic** digest of the install path rather than a per-run random value. The only two Cowork records available here (`plugin:lool-skills:lool-notion`, `…:lool-pitch-notes`) both name staged directories that **still exist**, and the staging root holds many sibling hashes, consistent with superseded ones lingering rather than being removed. That is n=2 and must not be read as a rate — but it is enough to say the 49/56 figure is a **CLI-lane measurement**, not a universal one, and that the two lanes fail differently or not at all. Do not carry the number across.
