...
| `CLAUDE_CODE_DISABLE_AGENTS_FLEET` (Desktop/nested-task sense) | env var | Set to `"1"` alongside `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS:"1"` in a Tasks-tool-child spawn path | Lesson 100 documents this name only as the *old*, renamed-away name for `CLAUDE_CODE_DISABLE_AGENT_VIEW` (Fleet view → agent view rename). This finding shows the old name is still live for a narrower purpose: suppressing nested Fleet/backgrounding specifically inside a Tasks-tool child spawn. Worth a follow-up note on lesson 100 clarifying the name persists in this one spawn path even though the primary UI toggle renamed. |
...
| `CLAUDE_CODE_DISABLE_AGENTS_FLEET` (Tasks-child sense) | env var | Tasks-tool child spawn | Old Fleet-view name, still live for nested-task Fleet/background suppression |
