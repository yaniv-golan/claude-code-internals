...
> **Superseded at Desktop 2.7032.0 (Ch52/L190).** From 2.7032.0 the host agent's cwd is `/var/empty` (or `<session>/host-cwd`), write- and read-denied; `hostCwd` no longer exists and a relative file-tool path is refused. What follows is correct for every earlier release, and the absolute host outputs path works on both.
